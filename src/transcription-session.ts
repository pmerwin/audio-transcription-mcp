...
export class TranscriptionSession {
  ...
  private chunkSeconds: number; // Store chunk duration for cost calculation
  ...
  private readonly INACTIVITY_AUTO_PAUSE_MINUTES = 30; // Auto-pause after 30 minutes of NO user interaction
  private inactivityPauseTriggered = false;
   ...
   */
  private checkLongSessionWarnings(): void {
    if (!this.status.lastInteractionTime || this.status.isPaused) return;
    ...
    if (minutesSinceLastInteraction >= this.INACTIVITY_AUTO_PAUSE_MINUTES && !this.inactivityPauseTriggered) {
      ...
      this.transcriptManager.appendSystemMessage(
        `⏰ TRANSCRIPTION AUTO-PAUSED: No user interaction detected for ${minutesSinceLastInteraction} minutes. ` +
